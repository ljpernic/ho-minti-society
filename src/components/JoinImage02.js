...
export const query = graphql`
  fragment JoinImage02 on MarkdownRemark {
    frontmatter {
      joinImage02 {
        alt
        image
...
export default class JoinImage02 extends Component {
  state = {
    loaded: false,
    isOpen: false,
    sliderImages: [],
        ...
        {images && images.length > 0 && (
          <div className="JoinImage">
            {images.map((image, index) => (
              <div
                className="JoinImage--Item"
                key={_kebabCase(image.alt) + '-' + index}
                onClick={() => this.isOpen(true, index)}
                onKeyDown={this.handleKeyDown}
                tabIndex={0}
                // NOT SURE IF THIS SHOULD BE ProjectImage02 OR ProjectImage
                aria-label="Toggle JoinImage" 
                role="button"
              >
                <div>
                  <Image
